<?php
    ...
    require_once("../model/error_client.php");

    $control = $_GET['control'] ;

    $errorcl = new Error_client($conexion) ;

    switch ($control) {
        case 'consulta':
            $veconsulta = $errorcl->consulta() ;
        break ;
        case 'insertar':
            ...
            //print_r($params) ;
            $veconsulta = $errorcl->insertar($params) ;
        break ;
        case 'eliminar':
            //parametro de prueba: 422
            $id = $_GET['id'] ;
            $veconsulta = $errorcl->eliminar($id) ;
        break ;
        case 'editar':
            //siempre activa, desactivar al realizar una prueba
            $json = file_get_contents('php://input') ;
            ...
            //parametro de prueba: 422
            $id = $_GET['id'] ;
            $veconsulta = $errorcl->editar($id, $params) ;
        break ;
        case 'filtrar':
            $valor = $_GET['valor'] ;
            //parametro de prueba: 422 - Error+de+base+de+datos
            $veconsulta = $errorcl->filtrar($valor) ;
        break ;
