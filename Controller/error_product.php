<?php
    ...
    require_once("../model/error_product.php");

    $control = $_GET['control'] ;

    $errorpr = new Error_product($conexion) ;

    switch ($control) {
        case 'consulta':
            $veconsulta = $errorpr->consulta() ;
        break ;
        case 'insertar':
            ...
            //print_r($params) ;
            $veconsulta = $errorpr->insertar($params) ;
        break ;
        case 'eliminar':
            //parametro de prueba: 422
            $id = $_GET['id'] ;
            $veconsulta = $errorpr->eliminar($id) ;
        break ;
        case 'editar':
            //siempre activa, desactivar al realizar una prueba
            $json = file_get_contents('php://input') ;
            ...
            //parametro de prueba: 422
            $id = $_GET['id'] ;
            $veconsulta = $errorpr->editar($id, $params) ;
        break ;
        case 'filtrar':
            $valor = $_GET['valor'] ;
            //parametro de prueba: 422 - Error+de+base+de+datos
            $veconsulta = $errorpr->filtrar($valor) ;
        break ;
