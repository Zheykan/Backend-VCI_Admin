<?php
    ...
    require_once("../model/error_user.php");

    $control = $_GET['control'] ;

    $erroru = new Error_user($conexion) ;

    switch ($control) {
        case 'consulta':
            $veconsulta = $erroru->consulta() ;
        break ;
        case 'insertar':
            ...
            //print_r($params) ;
            $veconsulta = $erroru->insertar($params) ;
        break ;
        case 'eliminar':
            //parametro de prueba: 422
            $id = $_GET['id'] ;
            $veconsulta = $erroru->eliminar($id) ;
        break ;
        case 'editar':
            //siempre activa, desactivar al realizar una prueba
            $json = file_get_contents('php://input') ;
            ...
            //parametro de prueba: 422
            $id = $_GET['id'] ;
            $veconsulta = $erroru->editar($id, $params) ;
        break ;
        case 'filtrar':
            $valor = $_GET['valor'] ;
            //parametro de prueba: 422 - Error+de+base+de+datos
            $veconsulta = $erroru->filtrar($valor) ;
        break ;
